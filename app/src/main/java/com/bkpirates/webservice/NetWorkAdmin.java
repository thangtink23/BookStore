...
public class NetWorkAdmin {
	...
	public HttpResponse makeRequestUpload(String url) throws ClientProtocolException, IOException {
		...
		nameValuePairList.add(new BasicNameValuePair("publisher", bookEntity.getPulisher()));
		nameValuePairList.add(new BasicNameValuePair("distribute", bookEntity.getGenre()));

	/*if (newPushlier == null) {
			Log.d(TAG, bookEntity.getPulisher());
		nameValuePairList.add(new BasicNameValuePair("oldpublisher", bookEntity.getPulisher()));

	} else {
			// insert new pub and distribute
		Log.d(TAG + "checked", newPushlier);
		nameValuePairList.add(new BasicNameValuePair("newpublisher", newPushlier));

		}
		if (newGenre == null) {
			Log.d(TAG, bookEntity.getGenre());
			nameValuePairList.add(new BasicNameValuePair("olddistribute", bookEntity.getGenre()));
		} else
		{
			Log.d(TAG + "checked", newGenre);
			nameValuePairList.add(new BasicNameValuePair("newdistribute", newGenre));
		}*/
		UrlEncodedFormEntity entity = new UrlEncodedFormEntity(nameValuePairList, "UTF-8");
		httpPost.setEntity(entity);
		return httpClient.execute(httpPost);
	}
	public HttpResponse makeRequestUpDate(String url) throws ClientProtocolException, IOException {
		HttpClient httpClient = new DefaultHttpClient();
		HttpPost httpPost = new HttpPost(url);
		List<NameValuePair> nameValuePairList = new ArrayList<NameValuePair>();
